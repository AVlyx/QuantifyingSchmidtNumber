from utils.load_store_results import load_results_in_range, SdpResult, Separability, save_result
from utils.range_and_canonical import range_canonical
...
def run_sdp_on_range(
    ...
    already_computed_points = [result.p for result in load_results_in_range(filename, range_)]
    ...
    for p in tqdm(left_to_compute):
        ...
        obj_min, _ = SDP(lam, state, dims, solver="qics", real=True)
        if obj_min - tol <= 0:
            save_result(filename, p, separability, 1, obj_min, 0, 1, 1)
            continue
        E1_lower = E_t_lower_lambda_r2(lam, obj_min)
        minSchmidt = len(lam)
        ...
        if sum(lam) >= 3:
            ...
            compute_upper = False
        if compute_upper:
            obj_max, _ = SDP_max(lam, state, dims)
