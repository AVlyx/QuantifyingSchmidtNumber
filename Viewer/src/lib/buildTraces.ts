...
function isKept(rec: Record1D, settings: Settings): boolean {
  return settings.showSeparable || rec.separable === 0;
}
...
function buildLineFigure(files: DataFile[], settings: Settings): Figure {
  ...
  for (const file of files) {
    ...
    const y = records.map((r) => (isKept(r, settings) ? floorValue(r.Etl, settings.zeroFloor) : null));
...
function buildHistogramFigure(files: DataFile[], settings: Settings): Figure {
  ...
  const maxValue = Math.max(settings.zeroFloor, ...lower, ...upper.filter((v): v is number => v !== null));
  ...
  return {
    ...
    layout: {
      ...
      legend: { orientation: 'h', x: 0, xanchor: 'left', y: 1.06, yanchor: 'bottom', font: { size: 12 } },
      xaxis: {
        type: 'category',
        automargin: true,
      ...
      yaxis: {
        title: { text: 'max E<sub>t</sub> lower (log scale)', font: { size: 13, color: INK_MUTED } },
        type: 'log',
        range: [bottom, top],
        exponentformat: 'power',
        showexponent: 'all',
